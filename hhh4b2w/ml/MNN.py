...
class MNNModel(MLModel):
    ...
    def setup(self):
        ...
        for i, proc in enumerate(self.multiclass_tag):
    ...
    def prepare_inputs(
        ...
        for dataset, files in input["events"][self.config_inst.name].items():
            ...
            for inp in files:
                ...
                class_index = None
                # Determine which tag is in the dataset
                for tag, idx in self.multiclass_tag.items():
                    if self.config_inst.get_dataset(dataset).has_tag(tag):
                        class_index = idx
                        break
                    # Fail if no class matched
                if class_index is None:
                    raise Exception(f"No matching tag found for dataset {dataset}")
        
                target = np.eye(len(self.multiclass_tag))[class_index]
                target = np.tile(target, (len(ml_inputs), 1))  # shape: (N=len(ml_events), 3)
                # from IPython import embed; embed()
                ...
                if MNN_inputs["weights"] is None:
                    MNN_inputs["weights"] = weights
                    MNN_inputs["inputs"] = ml_inputs
                    # DNN_inputs["inputs"] = events
                    MNN_inputs["target"] = target
                else:
                    MNN_inputs["weights"] = np.concatenate([MNN_inputs["weights"], weights])
                    # DNN_inputs["inputs"] = np.concatenate([DNN_inputs["inputs"], events])
                    MNN_inputs["inputs"] = np.concatenate([MNN_inputs["inputs"], ml_inputs])
                    MNN_inputs["target"] = np.concatenate([MNN_inputs["target"], target])
                # from IPython import embed; embed()
        #
        # shuffle events and split into train and validation part
        #
        inputs_size = sum([arr.size * arr.itemsize for arr in MNN_inputs.values()])
        logger.info(f"inputs size is {inputs_size / 1024**3} GB")

        shuffle_indices = np.array(range(len(MNN_inputs["weights"])))
        np.random.shuffle(shuffle_indices)

        n_validation_events = int(self.validation_fraction * len(MNN_inputs["weights"]))

        train, validation = {}, {}
        for k in MNN_inputs.keys():
            MNN_inputs[k] = MNN_inputs[k][shuffle_indices]

            validation[k] = MNN_inputs[k][:n_validation_events]
            train[k] = MNN_inputs[k][n_validation_events:]
    ...
    def train(
        ...
        n_outputs = len(self.multiclass_tag)
    ...
    def evaluate(
        ...
        for i, model in enumerate(models):
            ...
            # from IPython import embed; embed()
            if len(prediction[0]) != len(self.multiclass_tag):
                raise Exception(
                    f"Prediction shape {len(prediction[0])} does not match number of processes {len(self.multiclass_tag)}",
                )
        ...
        for i in range(self.folds):
            ...
            idx = ak.to_regular(ak.concatenate([ak.singletons(fold_indices == i)] * len(self.multiclass_tag), axis=1))
            outputs = ak.where(idx, predictions[i], outputs)
        
        if len(outputs[0]) != len(self.multiclass_tag):
            raise Exception(
                f"Output shape {len(outputs[0])} does not match number of processes {len(self.processes)}",
            )        
