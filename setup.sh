...
setup_hhh4b2w() {
    ...
    #   - "HHH4B2W_", for features provided by the analysis repository itself.
    # Check the setup.sh in columnflow for documentation of the "CF_" variables. The purpose of all
    # "HHH4B2W_" variables is documented below.
    #
    ...
    if [ "${HHH4B2W_SETUP}" = "1" ]; then
        >&2 echo "the hhh4b2w analysis was already succesfully setup"
    ...
    #
    # global variables
    # (HHH4B2W = hhh4b2w, CF = columnflow)
    #

    # start exporting variables
    export HHH4B2W_BASE="${this_dir}"
    export CF_BASE="${this_dir}/modules/columnflow"
    export CF_REPO_BASE="${HHH4B2W_BASE}"
    export CF_REPO_BASE_ALIAS="HHH4B2W_BASE"
    ...
    if [ "${CF_REMOTE_ENV}" != "1" ]; then
        ...
        cf_setup_interactive "${CF_SETUP_NAME}" "${HHH4B2W_BASE}/.setups/${CF_SETUP_NAME}.sh" || return "$?"
    fi
    ...
    export PATH="${HHH4B2W_BASE}/bin:${PATH}"
    export PYTHONPATH="${HHH4B2W_BASE}:${HHH4B2W_BASE}/modules/cmsdb:${PYTHONPATH}"

    # initialze submodules
    if [ -e "${HHH4B2W_BASE}/.git" ]; then
        local m
        for m in $( ls -1q "${HHH4B2W_BASE}/modules" ); do
            cf_init_submodule "${HHH4B2W_BASE}" "modules/${m}"
        done
    fi
    ...
    export LAW_HOME="${LAW_HOME:-${HHH4B2W_BASE}/.law}"
    export LAW_CONFIG_FILE="${LAW_CONFIG_FILE:-${HHH4B2W_BASE}/law.cfg}"
...
main() {
    ...
    if setup_hhh4b2w "$@"; then
        cf_color green "hhh4b2w analysis successfully setup"
        return "0"
    else
        local code="$?"
